...
try:
    from . import bare_task_prompts as bare_task_prompts_dataset
    from . import conversational_task_prompts as conversational_task_prompts_dataset
    from . import verbose_task_prompts as verbose_task_prompts_dataset
    from . import directive_task_prompts as directive_task_prompts_dataset
    from . import impersonal_task_prompts as impersonal_task_prompts_dataset
    from . import expert_task_prompts as expert_task_prompts_dataset
    ...
    import bare_task_prompts as bare_task_prompts_dataset  # type: ignore
    import conversational_task_prompts as conversational_task_prompts_dataset  # type: ignore
    import verbose_task_prompts as verbose_task_prompts_dataset  # type: ignore
    import directive_task_prompts as directive_task_prompts_dataset  # type: ignore
...
DatasetName = Literal[
    ...
    "task_only",
    "verbose_no_horizon",
    "directive_no_horizon",
    "impersonal_no_horizon",
    "expert_no_horizon",
]

DATASETS = {
    "conversational_no_time": bare_task_prompts_dataset,
    "task_only": conversational_task_prompts_dataset,
    "verbose_no_horizon": verbose_task_prompts_dataset,
    "directive_no_horizon": directive_task_prompts_dataset,
    "impersonal_no_horizon": impersonal_task_prompts_dataset,
    "expert_no_horizon": expert_task_prompts_dataset,
}
...
def append_prompt_variants(
    ...
    unit: str,
    unit_variant: UnitVariant,
    number_formats: tuple[NumberFormat, ...] = ("numeric",),
) -> None:
    """Append the requested number renderings for one prompt."""
...
def time_variants(value: int, unit: str) -> Iterable[tuple[int, str, UnitVariant]]:
    """Yield the configured horizon and its next-smaller-unit equivalent."""
    yield value, unit, "original"
    smaller = smaller_unit_value(value, unit)
    if smaller is not None:
        yield *smaller, "smaller"
...
def generate_task_dataset(
    ...
    dataset: str = "task_only",
    remove_time_constraints: bool = False,
    number_formats: tuple[NumberFormat, ...] | None = None,
) -> list[PromptRecord]:
    ...
    conditions. Explicit and indirect durations use numeric values by default;
    ``number_formats=("words",)`` supplies candidates for the sampled worded corpus.
    """
    formats = ("numeric",) if number_formats is None else tuple(dict.fromkeys(number_formats))
    if not formats or any(f not in NUMBER_FORMATS for f in formats):
        raise ValueError("number_formats must contain 'numeric' and/or 'words'")
    ...
    if record_builder is not None:
        ...
        if number_formats is not None:
            unsupported.append("number_formats")
        if unsupported:
    ...
    if randomize_template:
        for task, task_config in sorted(task_configs.items()):
            ...
            for unit in sorted(units):
                for value in time_configs:
                    for rendered_value, rendered_unit, unit_variant in time_variants(value, unit):
                        template_config = random.choice(template_configs)
                        append_prompt_variants(
                            records=records,
                            template=str(template_config["template"]),
                            template_id=str(template_config["id"]),
                            template_metadata=template_metadata(template_config),
                            task=task,
                            task_metadata=task_metadata,
                            base_value=value,
                            base_unit=unit,
        ...
        for template_config in template_configs:
            ...
            for task, task_config in sorted(task_configs.items()):
                ...
                for unit in sorted(units):
                    for value in time_configs:
                        for rendered_value, rendered_unit, unit_variant in time_variants(
                            value, unit
                        ):
                            append_prompt_variants(
                                records=records,
